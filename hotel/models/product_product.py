...
class ProductProduct(models.Model):
    ...
    def _search(self, args, offset=0, limit=None, order=None, count=False, access_rights_uid=None, ):
        ...
        if isinstance(checkin_date, str):
            checkin_date = fields.datetime.strptime(context.get("checkin_date"),
                                                    tools.DEFAULT_SERVER_DATETIME_FORMAT)
        if isinstance(checkout_date, str):
            checkout_date = fields.datetime.strptime(context.get("checkout_date"),
                                                     tools.DEFAULT_SERVER_DATETIME_FORMAT)
                ...
                for rm_line in room.room_line_ids:
                    if rm_line.status != "cancel":
                        if (checkin_date <= rm_line.check_in <= checkout_date) or \
                            ...
                            assigned = True
                        elif (rm_line.check_in <= checkin_date <= rm_line.check_out) or \
                                (rm_line.check_in <= checkout_date <= rm_line.check_out):
                            assigned = True
                if not assigned:
                    avail_prod_ids.append(room.product_id.id)
            args.append(("id", "in", avail_prod_ids))
        return super(ProductProduct, self)._search(args, offset, limit, order, count=count,
