...
class HotelRoom(models.Model):
    ...
    room_categ_id = fields.Many2one("hotel.room.type", "Room Category", required=True, ondelete="restrict")
    room_amenities_ids = fields.Many2many("hotel.room.amenities", string="Room Amenities", ondelete="restrict",
...
class HotelRoomType(models.Model):
    ...
    child_ids = fields.One2many("hotel.room.type", "categ_id", "Room Child Categories", ondelete="restrict",)
    product_categ_id = fields.Many2one(
        "product.category", "Product Category", delegate=True, required=True, ondelete="restrict", copy=False
    )
        ...
        if name:
            ...
            if parents:
                ...
                if operator in expression.NEGATIVE_TERM_OPERATORS:
                    ...
                    domain = expression.OR([[("categ_id", "in", categories.ids)], domain])
                else:
                    domain = expression.AND([[("categ_id", "in", category_ids)], domain])
...
class HotelRoomAmenitiesType(models.Model):
    ...
    amenity_id = fields.Many2one("hotel.room.amenities.type", "Category", ondelete="restrict",)
    child_ids = fields.One2many("hotel.room.amenities.type", "amenity_id", "Amenities Child Categories")
    product_categ_id = fields.Many2one("product.category", "Product Category", delegate=True, required=True,
                                       ondelete="restrict", copy=False)

    @api.model
    def create(self, vals):
        if "amenity_id" in vals:
            amenity_categ = self.env["hotel.room.amenities.type"].browse(vals.get("amenity_id"))
...
class HotelRoomAmenities(models.Model):
    ...
    def write(self, vals):
        ...
        """
        if "amenities_categ_id" in vals:
            amenities_categ = self.env["hotel.room.amenities.type"].browse(vals.get("amenities_categ_id"))
            vals.update({"categ_id": amenities_categ.product_categ_id.id})
